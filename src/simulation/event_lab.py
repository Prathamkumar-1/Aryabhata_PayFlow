...
class EventRunRegistry:
    ...
    def _qwen_explanation(self, template: dict[str, Any], linked_intel: dict[str, Any]) -> str:
        ...
        return (
            f"Context guardrail for {OLLAMA_CFG.model}: preventive signal '{title}' is available "
            f"when the investigator reviews {template['title']}, but the model cannot execute "
            "holds, freezes, or threshold changes without analyst approval and PayFlow internal evidence."
        )
