...
# ── Test 0: Default Config Wiring ────────────────────────────────────────────

def test_default_config_is_nlu_config():
    from config.settings import UNSTRUCTURED_AGENT_CFG

    agent = _make_agent()
    assert agent._cfg is UNSTRUCTURED_AGENT_CFG
    assert hasattr(agent._cfg, "analysis_temperature")
    assert hasattr(agent._cfg, "max_analysis_tokens")
...
def test_config_and_exports():
    ...
    assert UNSTRUCTURED_AGENT_CFG.risk_modifier_scale == 0.35
...
if __name__ == "__main__":
    ...
    tests = [
        test_default_config_is_nlu_config,
