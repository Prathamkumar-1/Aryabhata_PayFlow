...
# ── Test 9: Evidence-Rationale Prompt Building ──────────────────────────────

async def test_evidence_rationale_prompt_building():
    """Prompt builders format alerts and evidence-rationale traces correctly."""
    ...
    # Test build_cot_prompt compatibility wrapper now emits bounded rationale instructions.
    cot = build_cot_prompt(
        thinking_so_far="Step 1: High velocity transfers detected.",
        new_evidence='{"mule_detected": true}',
    )
    legacy_directive = "/" + "think"
    assert legacy_directive not in cot
    assert "evidence-rationale mode" in cot
    assert "do not reveal hidden chain-of-thought" in cot
    ...
    _safe_print("  [PASS] test_evidence_rationale_prompt_building")


# ── Test 10: Agent Think Node (Mocked LLM) ─────────────────────────────────

async def test_agent_think_node_mock():
    """_think_node updates state with an evidence trace and parses tool calls."""
...
# ── Test 13: Evidence Cap for Unsupported Qwen Verdict ──────────────────────

async def test_qwen_verdict_without_evidence_is_capped():
    """Qwen high-risk JSON without evidence cannot trigger autonomous freeze."""
    cfg = InvestigatorAgentConfig(max_iterations=1)
    agent = InvestigatorAgent(llm_client=None, tool_executor=ToolExecutor(), config=cfg)
    unsupported_verdict = json.dumps({
        "verdict": "FRAUDULENT",
        "confidence": 0.97,
        "fraud_typology": "layering",
        "reasoning_summary": "High risk asserted without concrete evidence.",
        "evidence_cited": [],
        "recommended_action": "FREEZE",
    })

    def mock_call_llm(messages, tools=None):
        return {"content": unsupported_verdict, "tool_calls": []}
    ...
    assert verdict.confidence_source == "qwen_json_evidence_capped"
    assert "missing_evidence_capped" in verdict.llm_parse_status

    _safe_print("  [PASS] test_qwen_verdict_without_evidence_is_capped")
...
async def main():
    ...
    tests = [
        ...
        test_evidence_rationale_prompt_building,
        test_agent_think_node_mock,
        test_agent_tool_routing_mock,
        test_agent_full_loop_mock,
        test_qwen_verdict_without_evidence_is_capped,
