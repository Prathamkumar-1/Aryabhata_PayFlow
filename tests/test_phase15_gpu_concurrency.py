...
def test_gpu_concurrency_config_defaults():
    ...
    assert cfg.num_ctx_full == 8192, "num_ctx full"
    assert cfg.num_ctx_medium == 6144, "num_ctx medium"
...
async def test_dynamic_num_ctx_scales():
    q = _fresh_queue()
    assert q.current_num_ctx == 8192, "initial"
    await q.update_pressure(7600.0)
    assert q.current_num_ctx == 6144, "after HIGH"
    await q.update_pressure(7900.0)
    assert q.current_num_ctx == 4096, "after CRITICAL"
    await q.update_pressure(5000.0)
    assert q.current_num_ctx == 8192, "after return to NORMAL"
...
async def test_ctx_change_callback():
    ...
    await q.update_pressure(7600.0)  # NORMAL -> HIGH: 8192 -> 6144
    assert captured == [6144], f"Expected [6144], got {captured}"
    await q.update_pressure(7900.0)  # HIGH -> CRITICAL: 6144 -> 4096
    assert captured == [6144, 4096], f"Expected [6144, 4096], got {captured}"
...
async def test_llm_acquire_always_succeeds():
    ...
    async with q.acquire_llm() as num_ctx:
        assert num_ctx == 8192, f"Expected 8192 at NORMAL, got {num_ctx}"
