...
export function LivePipelineRibbon() {
  ...
  // Compact one-line summary shown when collapsed — keeps the live-pipeline
  // essence visible without occupying a dense 3-column band at all times.
  const summary = latest
    ? `${truncId(latest.txnId, 16)} · ${completion}% · ${truncId(latest.sender || '—', 8)} → ${truncId(latest.receiver || '—', 8)}${latest.amountPaisa > 0 ? ` · ${fmtPaisa(latest.amountPaisa)}` : ''}`
    : 'awaiting first event — launch a drill or inject a transaction'

  return (
    <section className="shrink-0 border-b border-border-subtle bg-bg-surface px-4 py-2 shadow-sm">
      {/* Collapse header — always visible, one line */}
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={() => setExpanded((v) => !v)}
          className="inline-flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-[0.14em] text-accent-primary hover:text-accent-hover transition-colors"
          aria-expanded={expanded}
        >
          {expanded
            ? <ChevronDown className="h-3.5 w-3.5" />
            : <ChevronRight className="h-3.5 w-3.5" />}
          <RadioTower className="h-3.5 w-3.5 text-alert-critical" />
          live backend pipeline
        </button>
        <span
          className={cn(
            'rounded-sm px-1.5 py-0.5 text-[8px] font-extrabold uppercase tracking-[0.1em]',
            connected ? 'bg-accent-primary/10 text-accent-primary' : 'bg-alert-critical/10 text-alert-critical',
          )}
        >
          {connected ? 'sse online' : 'connecting'}
        </span>
        {!expanded && (
          <span className="min-w-0 flex-1 truncate font-mono text-[10px] font-semibold text-text-secondary" title={summary}>
            {summary}
          </span>
        )}
      </div>

      {/* Expanded detail grid — the dense 3-column view, on demand */}
      {expanded && (
        <div className="mt-2 grid gap-2 2xl:grid-cols-[330px_minmax(0,1fr)_430px] animate-slide-up">
          <div className="min-w-0 rounded-md border border-accent-primary/20 bg-bg-elevated px-3 py-2">
            {latest ? (
              <div className="grid min-w-0 grid-cols-[1fr_auto] items-end gap-2">
                <div className="min-w-0">
                  <div className="truncate font-mono text-[11px] font-black text-text-primary" title={latest.txnId}>
                    {truncId(latest.txnId, 18)}
                  </div>
                  <div className="mt-0.5 truncate text-[9px] font-semibold text-text-muted">
                ...
                <div className="text-right">
                  <div className="font-mono text-[13px] font-black text-accent-primary">{completion}%</div>
                  <div className="text-[7px] font-bold uppercase tracking-[0.12em] text-text-muted">complete</div>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-[1fr_auto] items-end gap-2">
                <div>
                  <div className="font-mono text-[11px] font-black text-text-primary">awaiting first event</div>
                  <div className="mt-0.5 text-[9px] font-semibold text-text-muted">
                    Launch an event drill or inject a transaction to populate live stages.
                  </div>
                </div>
                <TimerReset className="h-5 w-5 text-text-muted/55" />
              </div>
            )}
          </div>

          <div className="min-w-0 rounded-md border border-border-subtle bg-bg-elevated px-3 py-2">
                ...
                return (
                  <div
                    ...
                    className={cn(
                      'min-w-0 rounded-md border px-2 py-1.5',
                        ...
                        ? 'border-accent-primary/25 bg-bg-surface text-accent-primary'
                  ...
                  >
                    <div className="flex items-center justify-between gap-1">
                      ...
                      {done ? <CheckCircle2 className="h-3 w-3 shrink-0 text-accent-primary" /> : <Activity className="h-3 w-3 shrink-0 text-text-muted/55" />}
                    </div>
                    <div className="mt-0.5 truncate text-[7px] font-semibold text-text-muted">
  ...
  return (
    <div className="min-w-0 rounded-md border border-border-subtle bg-bg-elevated px-3 py-2">
      <div className="mb-1 flex items-center gap-1.5">
        <Icon className={cn('h-3.5 w-3.5 shrink-0', active ? 'text-alert-critical' : 'text-text-muted/70')} />
        <span className="truncate text-[8px] font-extrabold uppercase tracking-[0.13em] text-accent-primary">{label}</span>
      </div>
      <div className="truncate text-[10px] font-black text-text-primary" title={value}>{value}</div>
      <div className="mt-0.5 truncate text-[8px] font-semibold text-text-muted" title={detail}>{detail}</div>
    </div>
  )
}
